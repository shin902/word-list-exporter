...
このアプリケーションは、以下のセキュリティ対策を講じています。

- **C**ORS (Cross-Origin Resource Sharing): `FRONTEND_URL` 環境変数で指定されたオリジンからのリクエストのみを許可します。
- **H**elmet: セキュリティ関連のHTTPヘッダーを適切に設定し、基本的なWeb脆弱性（XSS、クリックジャッキング等）からアプリケーションを保護します。
- **R**ate Limiting: APIエンドポイントへのリクエスト数を制限し、ブルートフォース攻撃やDoS攻撃から保護します。本番環境ではRedisによる永続的なレート制限が必須です。
- **I**nput Validation: Base64形式の画像データやリクエストボディのサイズを検証し、不正な入力や過大なペイロードを拒否します。
- **S**ecret Scanning: Gitフックを利用して、コミット前に機密情報（APIキー等）がコードに含まれていないかスキャンします。

### エラーハンドリング

- **S**ecure Error Handling: エラーレスポンスから機密情報（ファイルパス、スタックトレース等）を排除し、代わりにエラーIDを提供します。
- **L**ogging: 詳細なエラー情報はサーバーサイドでのみ記録されます。クライアントでエラーが発生した際は、`errorId`をサーバーログで検索することで、関連する詳細なエラー情報を特定できます。
